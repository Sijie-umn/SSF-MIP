...
def random_cv(cv_index, cv_year, roothpath, param_grid, num_random, model_name, device, subx_model):
    ...
    if model_name in ['CNN_LSTM', 'CNN_FNN']:
        ...
        valid_X = load_results(rootpath + 'test_X_subx_{}_forecast{}_{}.pkl'.format(cv_year, cv_index, subx_model))
        train_y = load_results(rootpath + 'train_y_subx_{}_forecast{}_{}.pkl'.format(cv_year, cv_index, subx_model))
        valid_y = load_results(rootpath + 'test_y_subx_{}_forecast{}_{}.pkl'.format(cv_year, cv_index, subx_model))
        # set input and output dim
        input_dim = train_X[0].shape[-1] + 1
        output_dim = train_y.shape[-1]
    ...
    for i in range(num_random):
        ...
        if model_name == 'EncoderDecoder':
            ...
            parameter_all.append(parameters)
            mdl = model.XGBMultitask_subx(num_models=output_dim, colsample_bytree=curr_colsample_bytree,
            ...
            mdl = model.LassoMultitask_subx(alpha=curr_alpha, num_models=output_dim, n_jobs=16, fit_intercept=False)
            mdl.fit(train_X, train_y)
            pred_y = mdl.predict(valid_X)
            history = None
        elif model_name == 'FNN':
    ...
    save_results(rootpath + 'cv_results_test/cv_results_' + model_name + '_{}_{}_{}.pkl'.format(cv_year, cv_index, subx_model), cv_results)
...
month_range = list(range(7, 13))

Parallel(n_jobs=-2)(delayed(random_cv)(cv_index, cv_year=year, roothpath=rootpath, param_grid=param_grid, num_random=num_random, model_name=model_name, device=device, subx_model=subx_model) for cv_index in month_range)
