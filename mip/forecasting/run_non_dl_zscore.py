...
def forecast_non_dl(month_id, year, rootpath, param_path, model_name, one_day):
    ...
    train_y = load_results(rootpath + 'zscore/train_y_pca_{}_forecast{}.pkl'.format(year, month_id))
    test_y = load_results(rootpath + 'zscore/test_y_pca_{}_forecast{}.pkl'.format(year, month_id))
    if one_day is True:
        train_X = train_X[:, -1, :]
        test_X = test_X[:, -1, :]
    ...
    best_parameter = load_results(param_path + 'zscore/{}_forecast{}.pkl'.format(model_name, month_id))
    ...
    save_results(rootpath + 'forecast_results/results_{}_{}_{}_zscore.pkl'.format(model_name, year, month_id), results)
