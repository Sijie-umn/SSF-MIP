...
class LassoMultitask_subx():
    ...
    def fit(self, X, y):
        ...
        """
        self.models = Parallel(n_jobs=self.n_jobs)(delayed(self.models[loc].fit)(np.concatenate((X[0], np.expand_dims(X[1][:, loc], axis=1)), axis=1), y[:, loc])
    ...
    def predict(self, X):
        ...
        """
        pred_y = np.asarray((Parallel(n_jobs=self.n_jobs)(delayed(self.models[loc].predict)(np.concatenate((X[0], np.expand_dims(X[1][:, loc], axis=1)), axis=1))
                                                          for loc in range(self.num_models))))  # 197x2x num_estimators
