...
for model in ['GMAO', 'NCEP']:
    for e in range(1, 5):
        tmp2m_week34 = pd.read_hdf('tmp2m_{}_e{}.h5'.format(model, e)).reset_index()
        ...
        tmp2m_week34.to_hdf('tmp2m_{}_week34_forecast_e{}.h5'.format(model, e), key='data')

        tmp2m_week34 = pd.read_hdf('tmp2m_{}_hindcast_e{}.h5'.format(model, e)).reset_index()
        tmp2m_week34_climo = pd.read_hdf('tmp2m_{}_climo.h5'.format(model))
        tmp2m_week34_climo['climo'] = tmp2m_week34_climo['tmp2m']
        tmp2m_week34_climo = tmp2m_week34_climo.drop('tmp2m', axis=1)
        tmp2m_week34['month'] = tmp2m_week34.start_date.dt.month
        ...
        tmp2m_week34 = tmp2m_week34.set_index(['lat', 'lon', 'start_date'])
        tmp2m_week34 = tmp2m_week34.sort_index()
        tmp2m_week34.to_hdf('tmp2m_{}_week34_hindcast_e{}.h5'.format(model, e), key='data')
